...
import { 
  ...
  Eye, FileText, Building2, ShieldAlert, Plus, Trash2, Phone, Briefcase, GraduationCap, AlertCircle, Save
} from 'lucide-react';
...
// --- HELPER COMPONENT: DURATION INPUT ---
const DurationInput = ({ label, value, onChange }: { label: string, value: string, onChange: (val: string) => void }) => {
...
// --- HELPER COMPONENT: SUGGESTION LIST (DROPDOWN) ---
const SuggestionList = ({ matches, onSelect, labelField, subLabelField, isVisible }: { matches: any[], onSelect: (item: any) => void, labelField: string, subLabelField: string, isVisible: boolean }) => {
    // Logic: Hanya render jika ada data matches DAN status isVisible true
    ...
    return (
        <div className="absolute top-full left-0 z-[100] w-full bg-white border border-slate-200 rounded-md shadow-xl mt-1 max-h-60 overflow-y-auto animate-in fade-in zoom-in-95 duration-200">
            ...
            {matches.map((item, idx) => (
                <div 
                    key={idx} 
                    className="p-3 hover:bg-blue-50 cursor-pointer border-b last:border-0 transition-colors flex justify-between items-start group"
                    // Menggunakan onMouseDown agar trigger sebelum onBlur input
                    onMouseDown={(e) => {
                        e.preventDefault(); 
                        onSelect(item);
                    }}
...
export default function OperatorRegistrasiTest() {
  ...
  // State untuk Focus Input (Agar list hanya muncul saat diklik)
  const [activeInput, setActiveInput] = useState<string | null>(null);

  // --- CONFIRMATION DIALOG STATE ---
  const [confirmDialog, setConfirmDialog] = useState<{
      isOpen: boolean;
      type: 'klien' | 'penjamin' | 'litmas' | null;
      payload: any | null; // Simpan form data di sini
      warningMessage?: string | null; // Pesan khusus (misal: penjamin kosong)
  }>({
      isOpen: false,
      type: null,
      payload: null,
  ...
  // --- DUPLICATE WARNING STATE (KHUSUS KLIEN) ---
  const [showDuplicateAlert, setShowDuplicateAlert] = useState(false);
  const [duplicatePayload, setDuplicatePayload] = useState<any>(null); // Temporary hold data

  // --- HELPER FUNCTIONS ---

  const checkLiveDuplicate = async (table: 'klien' | 'penjamin', field: string, value: string) => {
    if (!value || value.length < 3) {
        if (table === 'klien') setMatchesKlien([]);
        else setMatchesPenjamin([]);
        return;
    }
    ...
    try {
        ...
        if (table === 'klien') {
            setMatchesKlien(data || []);
        } else {
            setMatchesPenjamin(data || []);
        }
    } catch (err) {
  ...
  const removePerkara = (idx: number) => {
  ...
  const handleEditClick = async (item: any) => {
    setLoading(true);
    // Reset matches dan active input saat masuk mode edit
    setMatchesKlien([]);
    setActiveInput(null);
    try {
        ...
        if (firstLitmas?.perkara) setPerkaraList(firstLitmas.perkara);
        else setPerkaraList([]);
        setSelectedClientId(safeData.id_klien);
        if (firstLitmas) setSelectedPkId(firstLitmas.nama_pk || null);
  ...
  const handleCancelButton = () => {
    resetFormState();
    toast({ title: "Edit Dibatalkan", description: "Form direset." });
  };

  const resetFormState = () => {
    setEditingKlien(null); setEditingPenjamin(null); setEditingLitmas(null);
    setPerkaraList([]); setSelectedClientId(null); setSelectedPkId(null);
    setMatchesKlien([]); setMatchesPenjamin([]); setActiveInput(null);
    setTempPerkara({ pasal: '', tindak_pidana: '', nomor_putusan: '', vonis_pidana: '', denda: '', subsider_pidana: '', tanggal_mulai_ditahan: '', tanggal_ekspirasi: '' });
  };
  ...
  const handleViewDetail = async (id_klien: number) => {
    setLoading(true);
    try {
        const { data, error } = await supabase.from('klien').select(`*, penjamin (*), litmas:litmas!fk_litmas_klien (*, upt (nama_upt), perkara (*), petugas_pk:petugas_pk!fk_litmas_pk (nama, nip))`).eq('id_klien', id_klien).single();
        if (error) throw error;
  ...
  // === 1. INITIATE FUNCTIONS (CEK VALIDASI & BUKA DIALOG) ===
  ...
  const initiateSaveKlien = (e: React.FormEvent<HTMLFormElement>) => {
      ...
      // Cek Duplikat khusus untuk mode "Create Baru"
      if (!editingKlien && matchesKlien.length > 0) {
          setDuplicatePayload(formData); // Simpan data sementara
          setShowDuplicateAlert(true);   // Tampilkan alert duplikat dulu
          return;
      }
  ...
  const initiateSaveLitmas = async (e: React.FormEvent<HTMLFormElement>) => {
      ...
      // AMBIL DATA DARI EVENT SEBELUM ASYNC
      const formData = new FormData(e.currentTarget);

      // VALIDASI KHUSUS: CEK APAKAH PENJAMIN SUDAH ADA?
      setLoading(true);
      ...
      let warning = null;
      if (count === 0) {
          warning = "PERHATIAN: Data Penjamin untuk klien ini BELUM DIISI!";
      }

      setConfirmDialog({
          isOpen: true,
          type: 'litmas',
          payload: formData, // Data form disimpan untuk dieksekusi nanti
          warningMessage: warning
      });
  };

  // === 2. EXECUTE FUNCTIONS (EKSEKUSI KE DB SETELAH KONFIRMASI) ===
  ...
  const executeSave = async () => {
      ...
      try {
          if (type === 'klien') {
              ...
              if (editingKlien) {
                  ...
                  if (editingLitmas && perkaraList.length > 0) {
                      ...
                      const perkaraPayloads = perkaraList.map(p => ({
                          id_litmas: editingLitmas.id_litmas,
                          pasal: p.pasal, tindak_pidana: p.tindak_pidana, nomor_putusan: p.nomor_putusan,
                  ...
                  toast({ title: "Berhasil", description: "Data Klien Diupdate." });
              ...
              if(editingLitmas) {
                  const { error } = await supabase.from('litmas').update(dataLitmas).eq('id_litmas', litmasId);
                  if(error) throw error;
                  toast({ title: "Berhasil", description: "Data Litmas Diupdate." });
              } else {
                  const { data, error } = await supabase.from('litmas').insert(dataLitmas).select('id_litmas').single();
                  if (error) throw error;
            ...
            <Button variant="outline" role="combobox" aria-expanded={openClientCombo} className="w-full justify-between bg-white border-slate-300 hover:bg-slate-50 h-14 shadow-sm" disabled={loading}>
              {selectedClientId ? (
                <div className="flex flex-col items-start text-left w-full overflow-hidden">
                   <span className="font-bold text-slate-900 truncate w-full text-base">{listKlien.find(k => k.id_klien === selectedClientId)?.nama_klien}</span>
                   <span className="text-xs text-slate-500 truncate w-full font-mono bg-slate-100 px-1.5 py-0.5 rounded inline-block mt-0.5 border">Reg: {listKlien.find(k => k.id_klien === selectedClientId)?.nomor_register_lapas || '-'}</span>
                </div>
              ) : (
  ...
  return (
    ...
    >
           ...
           <Card className={cn("border-t-4 shadow-sm", editingKlien ? "border-t-amber-500 bg-amber-50/30" : "border-t-slate-800")}>
             <CardHeader>
               <div className="flex items-center justify-between">
                 ...
                 {editingKlien && <Button variant="outline" size="sm" onClick={handleCancelButton}><XCircle className="w-4 h-4 mr-2" /> Batal</Button>}
               </div>
             </CardHeader>
             <CardContent>
                     ...
                     <div className="space-y-4">
                       ...
                       <div className="grid gap-2">
                           ...
                           <div className="relative">
                               <Input 
                                   name="nama_klien" 
                                   defaultValue={editingKlien?.nama_klien || ''} 
                                   required placeholder="Nama Klien" 
                                   onChange={(e) => checkLiveDuplicate('klien', 'nama_klien', e.target.value)}
                                   onFocus={() => setActiveInput('nama_klien')}
                                   ...
                                   <TooltipProvider>
                                    <Tooltip>
                                        <TooltipTrigger asChild>
                                            <div className="absolute right-3 top-2.5 text-orange-500 animate-pulse cursor-help">
                                                <AlertCircle className="w-5 h-5" />
                                            </div>
                                        </TooltipTrigger>
                                        <TooltipContent side="right" className="bg-orange-500 text-white border-0">
                                            <p>Data mirip ditemukan!</p>
                                        </TooltipContent>
                                    </Tooltip>
                                   </TooltipProvider>
                               )}
                               ...
                               <SuggestionList 
                                   matches={matchesKlien} 
                                   isVisible={activeInput === 'nama_klien'}
                       ...
                       <div className="grid gap-2">
                           ...
                           <div className="relative">
                               <Input 
                                   name="nik_klien" 
                                   defaultValue={editingKlien?.nik_klien || ''} 
                                   placeholder="Nomor Induk Kependudukan" 
                                   onChange={(e) => {
                                       const val = e.target.value.replace(/[^0-9]/g, '');
                                       e.target.value = val;
                                       checkLiveDuplicate('klien', 'nik_klien', val);
                                   }}
                                   onFocus={() => setActiveInput('nik_klien')}
                                   onBlur={() => setTimeout(() => setActiveInput(null), 200)}
                                   maxLength={16}
                                   autoComplete="off"
                                   inputMode="numeric"
                               ...
                               <SuggestionList 
                                   matches={matchesKlien} 
                                   isVisible={activeInput === 'nik_klien'}
                 ...
                 <div className="space-y-4 bg-red-50 p-6 rounded-lg border border-red-100">
                   ...
                   <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-end bg-white p-4 rounded shadow-sm">
                        ...
                        <div className="md:col-span-1">
                            <Button type="button" onClick={addPerkaraToList} size="icon" className="bg-red-600 hover:bg-red-700 w-full"><Plus className="w-5 h-5" /></Button>
                        </div>
                   </div>
                   ...
                   <div className="space-y-2">
                       {perkaraList.map((p, idx) => (
                           <div key={p.id || idx} className="flex items-center justify-between bg-white p-3 rounded border border-red-200 text-sm">
                               ...
                               <Button type="button" variant="ghost" size="sm" onClick={() => removePerkara(idx)} className="text-red-500 hover:bg-red-50"><Trash2 className="w-4 h-4" /></Button>
                           </div>
                       ))}
                ...
                <form onSubmit={initiateSavePenjamin} className="space-y-6 max-w-3xl mx-auto">
                    ...
                    <div className={cn("space-y-6", !selectedClientId && "opacity-50 pointer-events-none")}>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            {/* INPUT NAMA PENJAMIN LIVE SEARCH */}
                            <div className="grid gap-2">
                                <Label>Nama Penjamin</Label>
                                <div className="relative">
                                    <Input 
                                        name="nama_penjamin" 
                                        defaultValue={editingPenjamin?.nama_penjamin || ''} 
                                        required 
                                        onChange={(e) => checkLiveDuplicate('penjamin', 'nama_penjamin', e.target.value)}
                                        onFocus={() => setActiveInput('nama_penjamin')}
                                        onBlur={() => setTimeout(() => setActiveInput(null), 200)}
                                        autoComplete="off"
                                        className={cn(matchesPenjamin.length > 0 && "pr-10 border-orange-300 ring-orange-200 focus-visible:ring-orange-300")}
                                    />
                                    {matchesPenjamin.length > 0 && (
                                        <div className="absolute right-3 top-2.5 text-orange-500 animate-pulse">
                                            <AlertCircle className="w-5 h-5" />
                                        </div>
                                    )}
                                    <SuggestionList 
                                        matches={matchesPenjamin} 
                                        isVisible={activeInput === 'nama_penjamin'}
                            ...
                            {/* INPUT NIK PENJAMIN LIVE SEARCH */}
                            <div className="grid gap-2">
                                <Label>NIK Penjamin</Label>
                                <div className="relative">
                                    <Input 
                                        name="nik_penjamin" 
                                        defaultValue={editingPenjamin?.nik_penjamin || ''} 
                                        placeholder="Wajib Diisi" 
                                        required 
                                        onChange={(e) => {
                                            const val = e.target.value.replace(/[^0-9]/g, '');
                                            e.target.value = val;
                                            checkLiveDuplicate('penjamin', 'nik_penjamin', val);
                                        }}
                                        onFocus={() => setActiveInput('nik_penjamin')}
                                        onBlur={() => setTimeout(() => setActiveInput(null), 200)}
                                        maxLength={16}
                                        autoComplete="off"
                                        inputMode="numeric"
                                    ...
                                    <SuggestionList 
                                        matches={matchesPenjamin} 
                                        isVisible={activeInput === 'nik_penjamin'}
             ...
             <CardContent>
               <form onSubmit={initiateSaveLitmas} className="space-y-6 max-w-3xl mx-auto">
                 ...
                 <div className={cn("space-y-6", !selectedClientId && "opacity-50 pointer-events-none")}>
                   ...
                   <div className="flex justify-end pt-4"><Button type="submit" className="bg-blue-600 hover:bg-blue-700">{loading ? <Loader2 className="animate-spin"/> : "Simpan Litmas"}</Button></div>
                 </div>
               </form>
             </CardContent>
           </Card>
      ...
      {/* ALERT DIALOG: KONFIRMASI DATA DUPLIKAT SAAT SIMPAN KLIEN */}
      ...
      {/* DIALOG KONFIRMASI UNIVERSAL (UNTUK SEMUA SAVE) */}
      <AlertDialog open={confirmDialog.isOpen} onOpenChange={(open) => setConfirmDialog(prev => ({ ...prev, isOpen: open }))}>
        <AlertDialogContent className={cn("border-l-4", confirmDialog.warningMessage ? "border-l-red-600" : "border-l-blue-600")}>
            <AlertDialogHeader>
                <AlertDialogTitle className={cn("flex items-center gap-2", confirmDialog.warningMessage ? "text-red-700" : "text-blue-700")}>
                    {confirmDialog.warningMessage ? <ShieldAlert className="w-6 h-6" /> : <Save className="w-5 h-5" />}
                    {confirmDialog.warningMessage ? "Peringatan Validasi Data!" : "Konfirmasi Penyimpanan"}
                </AlertDialogTitle>
                <AlertDialogDescription className="text-slate-700 text-base">
                    {confirmDialog.warningMessage ? (
                        <span className="block bg-red-50 p-3 rounded-md border border-red-100 text-red-800 font-medium mt-2">
                            {confirmDialog.warningMessage}
                        </span>
                    ) : "Apakah seluruh data yang Anda masukkan sudah benar?"}
                    <br/>
                    <span className="block mt-2">Data akan disimpan ke dalam sistem MONALISA.</span>
                </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
                <AlertDialogCancel disabled={loading}>Batal</AlertDialogCancel>
                <AlertDialogAction onClick={executeSave} disabled={loading} className={cn(confirmDialog.warningMessage ? "bg-red-600 hover:bg-red-700" : "bg-blue-600 hover:bg-blue-700")}>
                    {loading ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : null}
                    {confirmDialog.warningMessage ? "Tetap Lanjutkan" : "Ya, Simpan Data"}
                </AlertDialogAction>
            </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
